...
if (!isset($_SESSION['game_started'])) {
    ...
    if (!isset($_SESSION['target_score'])) $_SESSION['target_score'] = 200;
...
<?php if (!isset($_SESSION['game_started'])): ?>
    <div class="setup-container">
        <h1>Flip7 Calculator</h1>
        <form method="POST" class="setup-form">
            <input type="text" name="player_name" placeholder="Nom du joueur" autofocus required>
            <button type="submit" name="add_player" class="btn-circle">+</button>
        </form>
        ...
        <div class="player-list">
            <?php foreach ($_SESSION['setup_players'] as $idx => $name): ?>
                <div class="avatar-setup">
                    ...
                    <a href="?remove_p=<?= $idx ?>" class="remove-p">×</a>
                </div>
            <?php endforeach; ?>
        </div>

        <?php if(count($_SESSION['setup_players']) > 0): ?>
        <form method="POST" class="start-form">
            <label>Objectif :</label>
            <input type="number" name="target_score" value="<?= $_SESSION['target_score'] ?>">
            <button type="submit" name="start_game" class="btn-win">Lancer la partie</button>
        </form>
        <?php endif; ?>
    </div>
        ...
        <div class="podium-list">
            <?php $rank = 1; foreach ($_SESSION['winner'] as $idx => $total): ?>
                <div class="rank rank-<?= $rank ?>">
                    ...
                    <strong><?= htmlspecialchars($_SESSION['players'][$idx]) ?></strong>
                    <span class="final-score"><?= $total ?> pts</span>
                </div>
            <?php $rank++; endforeach; ?>
        </div>
        <a href="?reset=1" class="btn-win" style="display:inline-block; margin-top:30px; text-decoration:none;">Nouvelle Partie</a>
    </div>
...
<?php else: ?>
    ...
    <main class="game-area">
        <div class="current-turn">
            ...
            <div class="controls">
                <a href="?action=stop" class="btn btn-win">Valider</a>
        ...
        <div class="table-wrapper">
            <table>
                <thead>
                    <tr><?php foreach ($_SESSION['players'] as $p): ?><th><div class="avatar-table"><?= htmlspecialchars($p) ?></div></th><?php endforeach; ?></tr>
                </thead>
                <tbody>
        ...
        <div class="footer-actions">
            <a href="?action=end_now" class="btn-secondary">Terminer la partie</a>
            <a href="?reset=1" class="btn-reset">Réinitialiser</a>
        </div>
    </main>
<?php endif; ?>
